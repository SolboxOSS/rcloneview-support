...
- **Column sorting:** click any header (Name, Size, Modified) to bubble multi-gigabyte offenders to the top and tackle the worst folders first.
...
| Latest RcloneView build | Newer builds bundle updated Drive API scopes and Compare fixes; confirm Help -> About shows the current release.              |
| Plus license (optional) | Required if you want in-UI filters for cache folders, temp exports, or specific extensions as described in the Compare guide. |
| Local cache SSD         | Sorting huge folder trees is faster when the VFS cache and temporary Compare data sit on SSD instead of spinning disk.        |
...
### 4. Sort, batch-select, and tag duplicates

Use the column headers to sort by Size or Modified. Combine this with the toolbar icons (Show left-only, Show different) to isolate duplicates:
...
| Education org cleaning teacher folders        | Sort by Modified to find stale classes, copy final syllabi to a compliance vault, delete duplicates flagged by **Different files** view.          |
...
- Export CSV reports (from the Compare toolbar) so stakeholders can sign off on the delete list.

## Troubleshooting & FAQ

| Symptom                                    | Likely cause                                | Fix                                                                                                         |
| ------------------------------------------ | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Compare never finishes                     | Drive API throttled by massive folder trees | Split the job into subfolders or enable Drive API "Increase per-user limit" in Google Admin.                |
| Filters not available                      | Using Free license                          | Upgrade to Plus or apply rclone CLI filters via job settings before Compare.                                |
| Deleted files reappear                     | Another user sync app is re-uploading       | Coordinate cleanup windows and pause Drive for Desktop/third-party sync clients while you delete.           |
| Duplicate names remain but show equal icon | Files share the same size                   | Use rclone's `dedupe` command from the Terminal pane for byte-level merging after Compare confirms matches. |
| Slow UI after hours of sorting             | Cache stored on slow HDD                    | Go to Settings -> General and move cache/temp paths to SSD, then remount remotes.                           |
